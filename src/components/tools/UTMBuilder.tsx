    ...
    <div className="flex flex-col gap-6 w-full max-w-3xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">URL Base *</label>
          <input
            type="url"
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            placeholder="https://ejemplo.com"
            className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:ring-2 focus:ring-blue-500 outline-none"
          />
        </div>
      ...
      <div className="mt-4 p-6 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700">
        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 flex items-center gap-2">
          <Link size={18} /> URL Generada:
        </label>
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-grow bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-300 dark:border-slate-600 break-all font-mono text-sm">
            {generatedUrl || 'Tu URL aparecerá aquí...'}
          </div>
          <button
